...
class Decoder(keras.layers.Layer):
    def __init__(self, units: int, attn_num_heads: int,
    ...
    def call(self, context, x, state=None, return_state=False):
        ...
        # lookup the embedding and process the target sequence
        x = self.embedding(x)
        x, state = self.rnn(x, initial_state=state)

        # use RNN output as the query for attention over the context
